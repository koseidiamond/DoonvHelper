...
public class CustomNPC : Actor
{
    ...
    /// </summary>
    public CustomNPC(EntityData data, Vector2 offset, EntityID id) : this(
    ...
    public override void Awake(Scene scene)
    {
        ...
        deterministicRandom = new Random(level.Session.Deaths * 37 + ID * 13 + (int)player.X * 101 + (int)player.Y);
        if (level.Session.GetFlag("CelesteTAS_TAS_Was_Run"))
        {
            // TAS info
            Logger.Log(LogLevel.Info, "DoonvHelper", $"NPC RNG seed: Deaths: {level.Session.Deaths}, EntityID: {ID}, PlayerPos: {player.Position}");
        }
    }
    ...
    {
        Collidable = false;
        if (Sprite.Has("gore") && !DoonvHelperModule.Settings.DisabledGore)
        {
        ...
        switch (AI)
        {
            ...
            case AIType.ChaseJump:
                ...
                if (nextMoveTimer < 0f)
                {
                    ...
                    nextMoveTimer = deterministicRandom.NextFloat() * (2f - 0.5f) + 0.5f;
                }
            ...
            case AIType.Wander:
                ...
                if (nextMoveTimer < 0f)
                {
                    nextMoveTimer = deterministicRandom.NextFloat() * (5f - 2f) + 2f;
                    float random = deterministicRandom.NextFloat();
                    if (random < (1f / 3f))
                        wanderAIdirection = 0f;
                    else
                        wanderAIdirection = Math.Sign(random - (2f / 3f));
                }
                WalkerFall();
                if (WalkerJumpCheck()) Velocity.Y = -JumpHeight;
